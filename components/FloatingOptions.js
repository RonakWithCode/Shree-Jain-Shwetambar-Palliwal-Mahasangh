    ...
    <div className="fixed right-0 top-1/2 transform -translate-y-1/2 z-50">
      <div className="flex flex-col space-y-2">
        {options.map((option, index) => (
          <div
            key={index}
            className={`${option.color} text-white p-3 rounded-l-lg shadow-lg cursor-pointer transition-all duration-300 flex items-center justify-end hover:translate-x-1`}
          >
            <span className="font-hindi text-sm sm:text-base whitespace-nowrap mr-2">
              {option.label}
            </span>
